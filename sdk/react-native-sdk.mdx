...
3. (선택) **Rich Push 알림 기능 활성화**  
  > 👉 [iOS Push Extension 설정](https://docs.notifly.tech/ko/developer-guide/client-sdk/advanced/ios-push-extension)
...
### (선택) iOS Push Extension 설정

<Info> `notifly-sdk` 버전 3.0.4 이상부터 지원됩니다. </Info>

Push Extension을 설정하면 리치 푸시(Rich Push) 기능을 사용할 수 있습니다. Notifly의 [Push Extension 가이드](advanced/rich-push-notification)를 참고해 설정하세요.
...
<Warning> Push Extension 설정을 하지 않아도 SDK는 정상 동작하지만, 리치 푸시 및 수신 트래킹은 비활성화됩니다. </Warning>

### 초기화 코드 추가

앱 진입점(`index.js` 또는 `App.js`)에서 초기화 코드를 추가합니다.
